...
+
서브넷에는 동일한 계층 3 서브넷에 속하는 IP 주소 풀이 포함되어 있습니다. 이들 파일은 System Manager 또는 명령을 사용하여 `network subnet create` 생성됩니다.
...
** CLI: 고급 권한 수준에서 명령을 사용하여 각 노드에서 지원되는 LIF 용량과 `network interface capacity details show` 명령을 사용합니다 `network interface capacity show`.
...
* ONTAP 9.10.1부터 `network interface` CLI 명령에 RDMA 기반 NFS 구성에 대한 매개 변수가 포함됩니다 `-rdma-protocols`. RDMA 기반 NFS 구성용 네트워크 인터페이스 생성은 ONTAP 9.12.1부터 System Manager에서 지원됩니다. 자세한 내용은 을 xref:https://docs.netapp.com/us-en/ontap/nfs-rdma/configure-lifs-task.html[RDMA를 통한 NFS에 대한 LIFS 구성]참조하십시오.
* ONTAP 9.11.1부터 ASA(All-Flash SAN 어레이) 플랫폼에서 자동 iSCSI LIF 페일오버를 사용할 수 있습니다.
+
지정된 SVM에 iSCSI LIF가 없거나 지정된 SVM에 있는 모든 기존 iSCSI LIF가 iSCSI LIF 페일오버에 이미 활성화되어 있는 경우, 새로 생성된 iSCSI LIF에서 iSCSI LIF 페일오버가 자동으로 사용되도록 설정됩니다(페일오버 정책이 `sfo-partner-only` 로 설정되고 자동 되돌리기 값이 로 `true` 설정됨).

+
ONTAP 9.11.1 이상으로 업그레이드한 후 SVM에 iSCSI LIF 페일오버 기능을 사용하지 않고 기존 iSCSI LIF가 있는 상태에서 동일한 SVM에서 새 iSCSI (`disabled`LIF를 생성하면 새 iSCSI LIF는 SVM에 있는 기존 iSCSI LIF의 동일한 페일오버 정책을 사용함)

+
link:https://docs.netapp.com/us-en/ontap/san-admin/asa-iscsi-lif-fo-task.html["ASA 플랫폼의 iSCSI LIF 페일오버"^]
...
. 을 image:icon_add.gif["추가 아이콘"]선택합니다.
...
....
+
** `-home-node` LIF에서 명령을 실행할 때 LIF가 반환되는 노드입니다. `network interface revert`
+
또한 LIF가 -auto-revert 옵션을 사용하여 홈 노드 및 홈 포트로 자동으로 되돌아가는지 여부를 지정할 수도 있습니다.

** `-home-port` LIF에서 명령을 실행할 때 LIF가 반환하는 물리적 또는 논리 포트입니다. `network interface revert`
** 및 `-netmask` 옵션을 사용하여 IP 주소를 지정하거나 옵션을 사용하여 서브넷에서 할당을 활성화할 `-subnet_name` 수 `-address` 있습니다.
...
** `-auto-revert` 시동, 관리 데이터베이스 상태 변경 또는 네트워크 연결이 이루어질 때 데이터 LIF가 홈 노드로 자동 복구되는지 여부를 지정할 수 있습니다. 기본 설정은 `false`이지만 환경의 네트워크 관리 정책에 따라 로 설정할 수 `true` 있습니다.
**  `-service-policy` ONTAP 9.5부터 옵션을 사용하여 LIF에 서비스 정책을 할당할 수 `-service-policy` 있습니다. LIF에 서비스 정책을 지정한 경우, 이 정책을 사용하여 LIF에 대한 기본 역할, 페일오버 정책 및 데이터 프로토콜 목록을 구성합니다. ONTAP 9.5에서는 서비스 정책이 인터클러스터 및 BGP 피어 서비스에 대해서만 지원됩니다. ONTAP 9.6에서는 여러 데이터 및 관리 서비스에 대한 서비스 정책을 작성할 수 있습니다.
** `-data-protocol` FCP 또는 NVMe/FC 프로토콜을 지원하는 LIF를 생성할 수 있습니다. IP LIF를 생성할 때는 이 옵션이 필요하지 않습니다.
...
+
 `network ndp prefix show`명령은 advanced 권한 수준에서 사용할 수 있습니다.

.. 형식을 사용하여 `prefix::id` IPv6 주소를 수동으로 구성합니다.
+
`prefix` 는 다양한 인터페이스에서 학습된 접두사입니다.

+
를 파생하려면 `id` 임의의 64비트 16진수 숫자를 선택합니다.
...
| IPv6 주소입니다 | 네트워크 ping 6 
|===
.예
다음 명령은 LIF를 생성하고 및 매개 변수를 사용하여 IP 주소 및 `-netmask` 네트워크 마스크 값을 `-address` 지정합니다.
